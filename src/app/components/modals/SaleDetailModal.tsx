    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm overflow-y-auto">
        <motion.div
          ...
          className="w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-stone-200 overflow-hidden my-8"
        >
          <div className="px-6 py-5 border-b border-stone-100 flex items-center justify-between bg-[#FBFBFA]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h2 className="text-lg font-extrabold text-[#171717] font-mono">Vente {sale.id.slice(0, 8)}</h2>
            ...
            <button onClick={onClose} className="w-8 h-8 rounded-xl bg-stone-100 hover:bg-stone-200 flex items-center justify-center text-stone-600 transition-colors">
              <X size={16} />
            </button>
          </div>
                ...
                {(sale.lines ?? []).map((line) => {
                  ...
                  return (
                    <div key={line.id} className="py-2.5 flex items-center justify-between">
                      <div>
                        <p className="text-xs font-bold text-[#171717]">{product?.name || 'Produit supprimé'}</p>
                        <p className="text-[11px] text-stone-500">
                          Quantité : <strong>{line.quantity}</strong> × {formatCurrency(line.unitPrice)}
                        </p>
                      </div>
                      <span className="text-xs font-extrabold text-[#171717]">{formatCurrency(line.lineTotal)}</span>
                    </div>
                  )
                })}
              </div>
