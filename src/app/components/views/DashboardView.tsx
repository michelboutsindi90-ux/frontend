  ...
  return (
    <div id="dashboard-view" className="space-y-8 pb-16">
      ...
      <motion.div
        ...
        className="relative rounded-3xl overflow-hidden border border-white/[0.1] shadow-2xl p-6 sm:p-7 min-h-[180px] flex flex-col justify-center"
      >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        <div
          className="p-6 rounded-3xl bg-[#FFF4BF]/70 border border-[#FFD43B]/50 shadow-[0_8px_30px_rgba(255,212,59,0.15)] flex flex-col justify-between"
        >
          <div className="w-11 h-11 rounded-2xl bg-[#FFD43B] flex items-center justify-center text-[#171717] shadow-sm">
            <TrendingUp size={22} className="stroke-[2.5]" />
          </div>
          <div className="mt-5">
            <p className="text-xs font-bold uppercase tracking-wider text-stone-700">Chiffre d'affaires</p>
            <h3 className="text-2xl lg:text-3xl font-display font-bold text-[#171717] tracking-tight mt-1">
        ...
        <div
          className="p-6 rounded-3xl bg-white border border-stone-200/80 shadow-[0_6px_24px_rgba(0,0,0,0.03)] flex flex-col justify-between"
        >
          <div className="w-11 h-11 rounded-2xl bg-[#F6F6F3] flex items-center justify-center text-[#171717]">
            <ShoppingBag size={22} />
          </div>
          <div className="mt-5">
            <p className="text-xs font-bold uppercase tracking-wider text-[#777777]">Ventes</p>
            <h3 className="text-2xl lg:text-3xl font-display font-bold text-[#171717] tracking-tight mt-1">
        ...
        >
          <div className="flex items-start justify-between">
            ...
            {(stats?.totals.lowStockCount ?? 0) > 0 && (
              ...
              >
                <AlertTriangle size={12} /> {stats?.totals.lowStockCount} en stock faible
              </span>
            )}
          </div>
          <div className="mt-5">
            <p className="text-xs font-bold uppercase tracking-wider text-[#777777]">Produits Actifs</p>
            <h3 className="text-2xl lg:text-3xl font-display font-bold text-[#171717] tracking-tight mt-1">
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          <div>
            <h2 className="text-xl font-extrabold text-[#171717] tracking-tight">Évolution des ventes</h2>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="bg-[#F6F6F3] p-1 rounded-2xl flex items-center text-xs font-bold">
              <button
                ...
                className={`px-3 py-1.5 rounded-xl transition-all ${chartMetric === 'revenue' ? 'bg-[#FFD43B] text-[#171717] shadow-xs' : 'text-stone-600 hover:text-[#171717]'}`}
              >
                Chiffre d'affaires
              </button>
              <button
                onClick={() => setChartMetric('salesCount')}
                className={`px-3 py-1.5 rounded-xl transition-all ${chartMetric === 'salesCount' ? 'bg-[#171717] text-white shadow-xs' : 'text-stone-600 hover:text-[#171717]'}`}
              >
                Nombre de ventes
              </button>
            </div>
            ...
            <div className="bg-[#F6F6F3] p-1 rounded-2xl flex items-center text-xs font-bold">
              {(
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl transition-all ${period === t.id ? 'bg-white text-[#171717] shadow-xs' : 'text-stone-500 hover:text-stone-900'}`}
                >
                  {t.label}
                </button>
              ))}
        ...
        >
          ...
          <div className="divide-y divide-stone-100">
            ...
            {recentSales.map((sale) => {
              ...
              return (
                <div key={sale.id} onClick={() => onSelectSale(sale)} className="py-3 flex items-center justify-between cursor-pointer hover:bg-[#FBFBFA] -mx-2 px-2 rounded-xl transition-colors">
                  <div>
                    <p className="text-xs font-bold text-[#171717] font-mono">{sale.id.slice(0, 8)}</p>
                    <p className="text-[11px] text-stone-400">{new Date(sale.createdAt).toLocaleString('fr-FR')}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-bold text-[#171717]">{formatCurrency(sale.totalAmount)}</span>
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${badge.bg}`}>{badge.label}</span>
              ...
              <div key={prod.id} className="p-3 rounded-2xl bg-[#F6F6F3] flex items-center justify-between gap-3">
                ...
                {canManage && (
                  <button
                    ...
                    className="px-2.5 py-1.5 rounded-xl bg-white border border-stone-200 hover:bg-[#FFD43B] text-[11px] font-bold text-[#171717] hover:text-[#171717] transition-colors shrink-0"
                  >
                    Réassort
                  </button>
                )}
